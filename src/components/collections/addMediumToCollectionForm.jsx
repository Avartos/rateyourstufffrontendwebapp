...
const AddMediumToCollectionForm = ({ mediumId }) => {
  ...
  const fetchCollections = () => {
    fetch(`http://localhost:5000/rest/collections/user/${helper.getUserId()}`)
      .then((res) => {
        if (!res.ok) {
  ...
  const handleAddToCollection = () => {
    fetch(`http://localhost:5000/rest/collections/${currentCollection}/medium/${mediumId}`, {
        method: "PUT",
        headers: {
    ...
    <div className="addMediumToCollectionForm">
      ...
      <div className="collectionSelection">
        <Autocomplete
          ...
          style={{ width: 300 }}
          onChange={(e, value) => {setCurrentCollection(value ? value.id : null)}}
          renderInput={(params) => (
            <TextField {...params} label="Combo box" variant="outlined" />
          )}
        />
        {currentCollection && <Button onClick={handleAddToCollection}>Hinzufügen</Button>}
      </div>
    </div>
  );
};
